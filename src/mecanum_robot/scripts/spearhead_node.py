...
class IntegratedFollowerNode:
    def __init__(self):
        ...
        self.center_tolerance  = 15.0  # Pixel error minimal untuk mulai centering (mencegah getar)
        self.missed_frames     = 0     # Counter jika objek tidak terdeteksi sementara
        self.max_missed_frames = 5     # Batas maksimal frame hilang sebelum robot dihentikan
    ...
    def image_callback(self, data):
        ...
        # PERBAIKAN 1: Cari target dengan kotak (area) TERBESAR
        # Ini mencegah robot tertipu oleh noise kecil di latar belakang
        ...
        if best_box is not None:
            # Objek Ditemukan!
            self.missed_frames = 0 # Reset counter memori frame
            
            xmin, ymin, xmax, ymax = best_box
            box_center_x = (xmin + xmax) / 2.0
            box_area = max_area
            
            # --- LOGIKA KOREKSI POSISI TENGAH (ROTASI) ---
            error_x = self.center_x_image - box_center_x
            
            # Berikan toleransi ("deadband") agar robot tidak bergetar kiri-kanan
            if abs(error_x) > self.center_tolerance:
                ...
                angular_z = 0.0
                self.integral_error_x = 0.0 # Reset integral jika sudah lurus
                
            twist_msg.angular.z = angular_z

            # --- LOGIKA MAJU/MUNDUR MENDEKATI OBJEK ---
            if box_area < self.stop_area:
                ...
                # PERBAIKAN 2: angular.z dibiarkan tetap memproses meskipun linear_x = 0
                # Robot akan tetap meluruskan posisinya walau sudah cukup dekat.

            self.last_twist = twist_msg # Simpan perintah berhasil terakhir
            
        else:
            # PERBAIKAN 3: Toleransi kehilangan frame
            self.missed_frames += 1
            if self.missed_frames < self.max_missed_frames:
                # Jika baru kehilangan beberapa frame, pertahankan pergerakan terakhir
                twist_msg = self.last_twist
            else:
                # Jika benar-benar hilang (melewati batas frame), baru hentikan paksa
                self.integral_error_x = 0.0
                twist_msg.linear.x = 0.0
        ...
        # Gambar penanda box terbesar (Biru) untuk membedakan dari false positive
        if best_box is not None:
